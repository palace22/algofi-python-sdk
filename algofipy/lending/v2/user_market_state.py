...
class UserMarketState:
    def __init__(self, market, state):
        ...
        self.b_asset_collateral = state.get(
            MARKET_STRINGS.user_active_b_asset_collateral, 0
        )
        self.b_asset_collateral_underlying = market.b_asset_to_asset_amount(
            self.b_asset_collateral
        )
        self.borrow_shares = state.get(MARKET_STRINGS.user_borrow_shares, 0)
        self.borrowed_underlying = market.borrow_shares_to_asset_amount(
            self.borrow_shares
        )
        self.rewards_states = []
...
class UserRewardsState:
    def __init__(self, state, program_index):
        ...
        program_index_key = (
            MARKET_STRINGS.user_rewards_program_number_prefix + program_index_bytestr
        )
        latest_rewards_index_key = (
            MARKET_STRINGS.user_latest_rewards_index_prefix + program_index_bytestr
        )
        unclaimed_rewards_key = (
            MARKET_STRINGS.user_unclaimed_rewards_prefix + program_index_bytestr
        )

        self.rewards_program_number = state.get(program_index_key, 0)
        self.latest_rewards_index = bytes_to_int(
            b64decode(state.get(latest_rewards_index_key, 0))
        )
