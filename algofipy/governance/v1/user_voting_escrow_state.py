...
class UserVotingEscrowState:
    def __init__(self, user_local_state):
        ...
        self.amount_locked = user_local_state.get(
            VOTING_ESCROW_STRINGS.user_amount_locked, 0
        )
        self.lock_start_time = user_local_state.get(
            VOTING_ESCROW_STRINGS.user_lock_start_time, 0
        )
        self.lock_duration = user_local_state.get(
            VOTING_ESCROW_STRINGS.user_lock_duration, 0
        )
        self.amount_vebank = user_local_state.get(
            VOTING_ESCROW_STRINGS.user_amount_vebank, 0
        )
        self.boost_multiplier = user_local_state.get(
